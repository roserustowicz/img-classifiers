...
def load_data():
    ...
    X_val = X_val*1.0/np.max(X_val) # normalize to 1
...
def create_model(num_classes):
    ...
    # Parameters should be tweaked for performance 
...
def main():
    ...
    # Tweak based on validation performance
    score_val = model.evaluate(X_val, y_val, verbose=1)
    print('Val loss: ', score_val[0])
    print('Val accuracy: ', score_val[1])

    # DO NOT tweak based on test performance ... this should be evaluated once at the end once the model is set
...
if __name__ == '__main__':
    ...
    main_eval()  # evaluates a loaded model
