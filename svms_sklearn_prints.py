...
#X_tr = np.load('../datasets/kings_05_rapideye/train_data.npy')
...
#X_val = np.load('../datasets/kings_05_rapideye/val_data.npy')
...
#X_test = np.load('../datasets/kings_05_rapideye/test_data.npy')
...
for C in [1000000]:
    for gamma in [0.1]: #[0.001, 0.01]:
        ...
        # Performance should be tuned and evaluated based on performance over the validation dataset
        y_val_pred = rbf_svc.predict(X_val)
        print('Result on validation set')
        print(classification_report(y_val, y_val_pred))
        print('Acc:')
        print(accuracy_score(y_val, y_val_pred))
        # DO NOT tune parameters based on test performance. This should only be run once the model 
        #   is defined to give a final estimate of algorithm performance
